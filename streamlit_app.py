...
# 3. 核心數據抓取與清洗 
# ==========================================
@st.cache_data(ttl=600) # 10分鐘快取，確保頻繁點擊不會被鎖IP
...
def show_help_dialog():
    ...
    * 持股最低成本限制：**{MIN_PORTFOLIO_COST/10000:,.0f} 萬元**。
    
    #### 2. 稅費與風控
    * 法人級手續費：**0.04%**。
    * 總虧損達 2,000 萬或階段虧損達 1,000 萬，強制停止交易。
    * 標的損失達 30%，須於次日強制出清。
    
    #### 3. 每日結算操作
    * **強烈建議學員於每日 15:00 後登入**，此時點擊[更新收盤價]，系統抓取的才會是正確的「今日收盤價」。
    * 每日離開前點擊 **[📥 結算今日淨值]** 紀錄軌跡，並下載 JSON 進度檔。
    """)
...
# 5. UI 佈局：頂部儀表板與風控
# ==========================================
st.title(f"📈 STP 操盤手模擬平台 - {st.session_state.group}")
...
if total_pnl <= -TOTAL_LOSS_LIMIT:
    ...
    st.error("🚨 警告：本階段虧損已達 1,000 萬，須於次日出清並暫停交易！")
    is_phase_halt = True

m1, m2, m3, m4 = st.columns(4)
m1.metric("帳戶總淨值", f"${eq:,.0f}")
m2.metric("可用現金", f"${st.session_state.cash:,.0f}")
m3.metric("交易員損益 (Total)", f"${total_pnl:,.0f}", delta=f"{total_pnl:,.0f}")
m4.metric("當前持股成本", f"${total_cost:,.0f}")

if 0 < total_cost < MIN_PORTFOLIO_COST:
    st.warning("⚠️ 提醒：持股總成本尚未達 2,000 萬最低規範水位！")
...
with t_col:
    ...
    if is_phase_halt:
        ...
        with st.form("trade_form", clear_on_submit=True):
            ...
            s_name = info.get('name', "請先更新報價或輸入代號")
            m_price_ref = info.get('price', 0.0)
            is_etf = info.get('is_etf', False)
            
            st.caption(f"🔍 標的: {s_name} | 市價參考: {m_price_ref}")
            price = st.number_input("成交單價", min_value=0.0, value=float(m_price_ref), step=0.1)
            qty = st.number_input("數量 (股)", min_value=1, step=1000, value=1000)
            note = st.text_area("交易理由 (必填)")
            ...
            if do_buy or do_sell:
                ...
                if not ticker or price <= 0: st.error("請輸入正確標的與單價")
                elif not note: st.error("📝 依據規範，必須填寫交易理由！")
                elif not group_valid: st.error(f"❌ 違規：{st.session_state.group} 不得操作此類商品！")
                else:
                    ...
                    if do_buy:
                        ...
                        if (cur_cost + net_cost) > COST_LIMIT_PER_TICKER:
                            st.error(f"❌ 攔截：總成本將超過法人風控 4,000 萬上限！")
                            ...
                            st.session_state.trades.append({"time": datetime.now().strftime("%m/%d %H:%M"), "action": "買進", "ticker": ticker, "name": s_name, "price": price, "qty": qty, "note": note, "rule": exec_note})
                            st.rerun()
                    ...
                    if do_sell:
                        if ticker not in st.session_state.positions or st.session_state.positions[ticker]['quantity'] < qty: 
                            ...
                            st.session_state.trades.append({"time": datetime.now().strftime("%m/%d %H:%M"), "action": "賣出", "ticker": ticker, "name": s_name, "price": price, "qty": qty, "note": note, "rule": exec_note})
                            st.rerun()
    ...
    with t1:
        if st.session_state.positions:
            ...
            for t, p in st.session_state.positions.items():
                ...
                status = "🚨 30%強制平倉" if ratio <= -0.3 else "正常"
                disp_p.append({
                    "標的": t, "名稱": st.session_state.market_prices.get(t, {}).get('name', '-'),
                    "數量": p['quantity'], "均價": round(p['avg_cost'], 2), 
                    "市價": cur_p, "報酬率": f"{ratio:.2%}", "狀態": status
                })
            st.dataframe(pd.DataFrame(disp_p), use_container_width=True, hide_index=True)
            if any("強制平倉" in str(x) for x in disp_p):
